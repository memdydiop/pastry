...
@php
    ...
    $iconClasses = Flux::classes('text-sidebar-item-color/35 group-hover:text-sidebar-item-hover-color')
        ->add($iconVariant === 'outline' ? 'size-4' : '');

    $classes = Flux::classes()
        ->add('group flex items-center')
        ->add('rounded has-data-[circle=true]:rounded-full')
...
<button type="button" {{ $attributes->class($classes) }} data-flux-profile>
    ...
    <?php if ($name): ?>
        <span class="mx-2 text-sm text-sidebar-item-color/35 group-hover:text-sidebar-item-hover-color font-medium truncate">
            {{ $name }}
        </span>
    <?php endif; ?>
